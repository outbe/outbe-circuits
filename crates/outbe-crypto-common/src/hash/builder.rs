...
//!
//! Every outbe entity hash is structurally the same computation: take
//! the first two inputs and Poseidon2-hash them, then for each
//! subsequent input chain `state = Poseidon2(state, next)`.
//! [`OutbeHasher`] packages that pattern into a fluent builder so each
//! formula reads as a sequence of `absorb*` calls on a seed.
//!
//! The seed is **not** `Fr::ZERO`. A two-input hash
...
//! construction: `OutbeHasher::new(owner).absorb(wwd).finalize()`
...
//! `outbe-crypto-common` free of any ABI dependency.
...
/// `state = Poseidon2(state, input)`. [`OutbeHasher::finalize`]
...
impl OutbeHasher {
    ...
    /// Absorb a single `Fr`.
    pub fn absorb(self, value: Fr) -> Result<Self, OutbeCryptoError> {
        Ok(Self {
    ...
    pub fn absorb_u64(self, value: u64) -> Result<Self, OutbeCryptoError> {
        self.absorb(Fr::from(value))
    }

    /// Absorb a `u128` by widening to `Fr`.
    pub fn absorb_u128(self, value: u128) -> Result<Self, OutbeCryptoError> {
        self.absorb(Fr::from(value))
    }

    /// Absorb a slice of `Fr` in order.
    pub fn absorb_many(mut self, values: &[Fr]) -> Result<Self, OutbeCryptoError> {
        for v in values {
            self = self.absorb(*v)?;
        }
        Ok(self)
...
/// building block for [`OutbeHasher`]. Exposed for cases where a
/// caller genuinely needs a one-shot pair hash without a builder.
pub fn poseidon2(a: Fr, b: Fr) -> Result<Fr, OutbeCryptoError> {
    let mut poseidon = outbe_poseidon::Poseidon::<Fr>::new_circom(2)
        .map_err(|e| OutbeCryptoError::Poseidon(format!("new_circom(2): {e}")))?;
    poseidon
        .hash(&[a, b])
        .map_err(|e| OutbeCryptoError::Poseidon(format!("hash: {e}")))
}
...
mod tests {
    ...
    fn finalize_without_absorb_returns_seed() {
        ...
        assert_eq!(OutbeHasher::new(seed).finalize(), seed);
    }
    ...
    fn one_absorb_equals_poseidon2_seed_input() {
        ...
        let from_builder = OutbeHasher::new(seed).absorb(value).unwrap().finalize();
    ...
    fn absorb_u64_matches_absorb_fr() {
        let seed = Fr::from(1u64);
        let a = OutbeHasher::new(seed).absorb_u64(7).unwrap().finalize();
        let b = OutbeHasher::new(seed)
            .absorb(Fr::from(7u64))
            .unwrap()
            .finalize();
        assert_eq!(a, b);
    ...
    fn absorb_many_equals_iterated_absorb() {
        ...
        let chained = OutbeHasher::new(seed)
            .absorb(xs[0])
            .unwrap()
            .absorb(xs[1])
            .unwrap()
    ...
    fn matches_inline_iterated_poseidon2() {
        ...
        let actual = OutbeHasher::new(seed)
            .absorb_many(&inputs)
            .unwrap()
            .finalize();
