...
pub fn poseidon3(a: Fr, b: Fr, c: Fr) -> Result<Fr, OutbeCryptoError> {
    let mut poseidon = outbe_poseidon::Poseidon::<Fr>::new_circom(3)
        .map_err(|err| OutbeCryptoError::Poseidon(format!("new_circom(3): {err}")))?;
    poseidon
        .hash(&[a, b, c])
