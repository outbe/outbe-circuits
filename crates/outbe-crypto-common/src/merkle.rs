...
//!
//! Node hashes are 32 raw bytes — the encoding is **big-endian**, as
...
///
/// `node_hash` is the **big-endian** 32-byte encoding of the sibling's
/// Fr value, matching the circuit's witness format.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct MerklePathElement {
    /// Sibling hash (BE-encoded Fr).
    pub node_hash: [u8; 32],
...
mod tests {
    ...
    /// Inline replica of `compute_merkle_root` driving `outbe_poseidon`
    /// directly — guards against future divergence between the wrapper
    /// and the raw Poseidon call.
    fn original_inline(entity_hash: &Fr, merkle_path: &[MerklePathElement], depth: usize) -> Fr {
        let mut poseidon = outbe_poseidon::Poseidon::<Fr>::new_circom(2).unwrap();
        let mut current_hash = *entity_hash;
        for i in 0..depth {
