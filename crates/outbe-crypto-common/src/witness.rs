...
//! extraction, which would force `outbe-crypto-common` (and therefore
//! every on-chain precompile) to drag in elliptic-curve cryptography
//! it doesn't need.
//!
//! ## Debug redaction
//!
...
use crate::error::OutbeCryptoError;
...
///
/// The ownership value is a pre-computed Poseidon hash of the owner's
/// public-key coordinates plus a nonce (see
/// `outbe_crypto_common::ownership::compute_ownership`). It is stored
/// on the NFT and produced by this getter.
...
pub trait HashableNFT {
    ...
    fn hash(&self) -> Result<Fr, OutbeCryptoError>;
}

/// Trait for generating ZK witnesses from NFT types.
///
/// Generic over the context `Ctx` which determines both the required
/// inputs and the produced witness type. **Implementations live in the
/// integration crate** because building a witness requires extracting
...
pub trait GenerateWitness<Ctx> {
    ...
    fn generate_witness(&self, ctx: Ctx) -> Result<Self::Witness, OutbeCryptoError>;
}
...
/// Noir circuit (`outbe-circuit-core/src/ownership.nr`), so the
...
///
/// All `[u8; 32]` Fr fields are **big-endian**.
#[derive(Clone, Debug)]
pub struct OwnershipPublicInputs {
    /// Owner commitment (ownership proof output), 32-byte BE Fr.
    pub ownership: [u8; 32],
...
/// already binds the signature to it.
#[derive(Clone, Debug)]
