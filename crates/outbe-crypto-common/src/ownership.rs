...
//! `derivedOwner` value on every outbe NFT; the ZK circuit re-derives
...
/// derives them by interpreting the HKDF output of the shared-secret
/// derivation as a Grumpkin scalar, then computing
/// `shared_pk = shared_sk * G_Grumpkin`.
pub fn compute_ownership(pk_x: Fr, pk_y: Fr, nonce: Fr) -> Result<Fr, OutbeCryptoError> {
    poseidon3(pk_x, pk_y, nonce)
}

/// Alias retained for forward-compat with `outbe-zk-circuit-noir`
/// testing helpers that import a Grumpkin-flavored name. Same body
/// as [`compute_ownership`].
pub fn compute_ownership_grumpkin(pk_x: Fr, pk_y: Fr, nonce: Fr) -> Result<Fr, OutbeCryptoError> {
