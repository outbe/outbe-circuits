...
//!
//! `#[derive(Entity)]` routes a `Vec<T>` field through
//! [`outbe_zk_core::codec::SortedSet`], which folds `[len, e₀, e₁, …]`
//! with the elements strictly ascending. The length prefix kills (1); the
//! required ordering kills (2) and rejects duplicates.

use outbe_zk_core::entity::Entity as EntityTrait;
use outbe_zk_core::error::Error;
use outbe_zk_core::{Entity, Fr};

/// Two adjacent set-valued body fields: the shape the length prefix and the
/// strict ordering exist for. Without them, `sr` / `ar` flatten ambiguously —
...
fn sort_set_removes_duplicates() {
    ...
    assert_eq!(
        sort_set::<u64>(&[10, 20, 10, 20, 10]).unwrap(),
        vec![10, 20]
    );
    // De-duped output hashes fine, and equals the same set passed cleanly.
    let deduped = sort_set::<u64>(&[30, 10, 10, 20, 30]).unwrap();
