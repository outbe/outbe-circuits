# outbe-protocol-derive

[![crates.io](https://img.shields.io/crates/v/outbe-protocol-derive.svg)](https://crates.io/crates/outbe-protocol-derive)
...
`#[derive(Entity)]` for [`outbe-protocol`](../outbe-protocol) — give a typed NFT
struct its `Entity` (and optionally `Owned`) impl from per-field `#[outbe(...)]`
roles, so the canonical hash preimage is read off the struct definition instead
of a hand-built `Vec<Field>`. Published alongside `outbe-protocol` at the same
version.
...
Every field must carry exactly one `#[outbe(...)]` attribute — a consensus
preimage must never silently omit a field.

| Role | Meaning |
| ---- | ------- |
| `#[outbe(id_seed)]` | Exactly one field; seeds the id accumulator. Must be a single element. |
| `#[outbe(id_body)]` | Folded onto the seed to form the id. |
| `#[outbe(body)]` | The entity body, folded from the id. |
| `#[outbe(owner)]` | Marks the stored `derivedOwner`; emits the `Owned` impl. Single element; combine with `body`. |
...
group is sorted by `pos`. Missing a `#[outbe(...)]` on any field is a compile
error (use `#[outbe(skip)]` to exclude it intentionally).
...
```sh
TAG=v0.8.0
ARTIFACT=outbe-protocol-derive-${TAG#v}.crate
...
[MIT](../../LICENSE) — same as `outbe-protocol`.
