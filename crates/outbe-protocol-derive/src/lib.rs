...
//! [`outbe_protocol::protocol::entity::Entity`] (and optionally `Owned`)
//! impl from per-field `#[outbe(...)]` roles, so the canonical hash
//! preimage is read off the struct definition instead of a hand-built
//! `Vec<Field>`.
//!
//! Each named field declares one or more roles:
//!
//! - `#[outbe(id_seed)]` — exactly one field; seeds the id accumulator.
...
//! [`outbe_protocol::codec::SortedSet`]). The element `T` must be a
//! single [`outbe_protocol::FieldElement`], and the caller must supply the
...
//! - `#[outbe(owner)]`   — marks the stored `derivedOwner`; emits an
//!   `Owned` impl. Must be a `FieldElement`. Combine with `body`.
//! - `#[outbe(skip)]`    — explicitly excluded from the hash preimage.
//! - `#[outbe(pos = N)]` — explicit fold position within its group (`body`
...
fn parse_field_cfg(field: &syn::Field) -> syn::Result<FieldCfg> {
            ...
            match id.as_str() {
                ...
                other => {
                    return Err(meta.error(format!(
                        "unknown outbe key `{other}` (expected id_seed, id_body, body, owner, skip, limbed, or pos = N)"
                    )))
                }
            }
            Ok(())
    ...
    if !saw_outbe {
        return Err(syn::Error::new(
            field.span(),
            "every field must carry a `#[outbe(...)]` role (use `#[outbe(skip)]` to exclude it from the hash)",
        ));
    }
...
/// strictly ascending) via [`outbe_protocol::codec::SortedSet`] rather
...
fn expand(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    for f in fields {
        ...
        if roles.owner {
            if owner.is_some() {
                return Err(syn::Error::new(ident.span(), "duplicate `#[outbe(owner)]`"));
            }
    ...
    let (seed_ident, seed_ty) = seed.ok_or_else(|| {
        syn::Error::new(
            name.span(),
            "an entity needs exactly one `#[outbe(id_seed)]` field",
        )
    })?;
