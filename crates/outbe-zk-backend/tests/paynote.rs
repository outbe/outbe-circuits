...
fn note_commitment(chain_id: u64, serial: Fr, asset: Fr, amount: u64) -> Fr {
    hash_multi(
        "PAYNOTE_COMMITMENT",
...
fn nullifier(commitment: Fr, spend_key: Fr) -> Fr {
    hash_multi("PAYNOTE_NULLIFIER", &[commitment, spend_key])
}
...
fn paynote_partial_spend_prove_verify_round_trip() {
    ...
    let change_commitment = note_commitment(chain_id, note_serial(next_key), asset, 60);
