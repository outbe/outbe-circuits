...
//! both `OutbeV1` (Poseidon) and `Mock` (additive hash + additive-challenge
...
//! and their round-trips live in `outbe-zk-canonical`, tested there.
//!
...
use outbe_protocol::primitive::kdf::Kdf;
...
/// `Mock` suite that wires them in (same curve, same formulas as `OutbeV1`).
...
mod mock {
    ...
    use outbe_protocol::error::Error;
    use outbe_protocol::primitive::curve::Grumpkin;
    use outbe_protocol::primitive::exchange::EcdhEmbedded;
    use outbe_protocol::primitive::hash::FieldHasher;
    use outbe_protocol::primitive::kdf::Kdf;
    use outbe_protocol::primitive::signature::Schnorr;
    ...
    impl<F: PrimeField> Kdf<F> for AddKdf {
        const DOMAIN: &'static str = "Outbe/kdf/add/test";
...
/// seed and a flat body. The real entity types live in `outbe-integration`.
