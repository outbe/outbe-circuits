...
//! over an embedded curve whose signatures are exactly what the Outbe ownership
//! circuits verify. `Schnorr` is generic over the curve `C`; the Grumpkin
//! instantiation (`Schnorr<Grumpkin>`) is what `OutbeV1` selects.
...
/// The Schnorr signature scheme Outbe signs with, over an embedded curve `C`
/// (Grumpkin for `OutbeV1`). Pure Rust.
///
/// A `Schnorr<C>` signature is exactly what the Outbe ownership circuits verify
