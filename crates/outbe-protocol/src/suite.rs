...
//! [`crate::OutbeV1`] selection — not hard-coded anywhere.
...
pub trait Suite: 'static {
    ...
    // ---- Formulas (override to change a formula; defaults are the Outbe ones) ----

    /// `derivedOwner = Hash([pk.x, pk.y, nonce])` (Poseidon3 in Outbe).
    ...
    /// (Poseidon5 + uint256 limb split in Outbe). The leading [`Suite::DOMAIN`]
