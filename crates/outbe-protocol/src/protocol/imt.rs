...
//! `S::Hash([left, right])` (Poseidon2 for `OutbeV1`). It is generic over the
...
//! outbe-chain-research): same shape, same `Poseidon2(left, right)` node hash,
...
impl<S: Suite> Imt<S> {
    /// Parent node hash: `Hash([left, right])` (Poseidon2 for `OutbeV1`).
...
mod tests {
    ...
    fn append_matches_inclusion_path() {
        let mut tree = Imt::<OutbeV1>::new(8).unwrap();
    ...
    /// The depth-32 empty-tree root for `OutbeV1` must match the chain's pinned
    ...
    fn empty_root_matches_chain_constant() {
        let root = Imt::<OutbeV1>::empty_root(Fr::zero(), 32).unwrap();
