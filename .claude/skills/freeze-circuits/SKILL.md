...
description: Run the circuit-freeze workflow when Noir sources under crates/outbe-zk-canonical/noir/ have changed. Runs `cargo xtask freeze-circuits` (the only step that runs nargo/bb), which mints a new frozen version for any circuit whose ACIR or ABI changed, updates the manifest + resources, then surfaces the dirty files to commit together with the source change.
---
...
Prereq: the pinned Noir toolchain must be installed — `mise install` (nargo 1.0.0-beta.22 + bb 5.0.0-nightly.20260522, pinned in `mise.toml`). The xtask locates them via `$NARGO`/`$BB` or `~/.nargo/bin/nargo` and `~/.bb/bb`.
...
2. **Freeze.** Run `cargo xtask freeze-circuits`. For each circuit it prints one of `unchanged <module> @ <ver>` (ACIR *and* ABI identical → no-op), `minted <module> X -> Y (old -> deprecated)` (a new frozen version; the superseded one is set `deprecated`, keeping only its VK), or `minted <module> @ <version> (new)` for a module with no active entry yet; it ends with `N circuit(s) minted.`. The first barretenberg build is multi-minute — do not kill it.
...
4. **Do not commit** on the user's behalf unless they ask. End with a one-line summary of what was minted (or "0 minted — ACIR and ABI unchanged").
...
- A `0 minted` result on a source edit is normal: the noir optimizer can collapse an edit to the same ACIR, and with the ABI unchanged too, identity is unchanged and no new version is owed.
